...
// 		},
		...
		methods: {
			...
			readMessage(item) { //标记消息为已读
				...
				this.$api.supplierMessageApi.messageRead(params)
					.then(res => {
			...
			messageDelete(params, type) { //删除消息
				uni.showLoading({
					title: '提交中'
				});
				this.$api.supplierMessageApi.messageDelete(params)
					.then(res => {
						const result = res.data;
						uni.hideLoading();
						if(result.msg_code === 100000) {
			...
			getMessageList({isRead, type, perPage=10, currentPage=1} = {}) { //获取消息列表
				...
				return this.$api.supplierMessageApi.messageList(params)
					.then(res => {
						const result = res.data;
						uni.hideLoading();
						if(result.msg_code === 100000) {
