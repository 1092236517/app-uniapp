	...
	export default {
		...
		methods: {
			...
			getDemandDetail() { //获取联采需求详情
				...
				return this.$api.branchDemandApi.incompleteDemandDetail(params, this.demandId)
					.then(res => {
						const result = res.data;
						if(result.msg_code === 100000) {
							const demandInfo = result.response;
							const provinceId = demandInfo.delivery_address_province_id && parseInt(demandInfo.delivery_address_province_id);
							const cityId = demandInfo.delivery_address_city_id && parseInt(demandInfo.delivery_address_city_id);
							const regionId = demandInfo.delivery_address_district_id && parseInt(demandInfo.delivery_address_district_id);
